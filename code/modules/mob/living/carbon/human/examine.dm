...
*/
	...
	if(wear_shirt && !(SLOT_SHIRT in obscured))
		var/str = "[m3] [wear_shirt.get_examine_overview(user)]. "
	...
	if(wear_pants && !(SLOT_PANTS in obscured))
		...
		var/str = "[m3] [wear_pants.get_examine_overview(user)][accessory_msg]. "
		if(!wear_armor)
	...
	if(head && !(SLOT_HEAD in obscured))
		var/str = "[m3] [head.get_examine_overview(user)] on [m2] head. "
	...
	if(wear_armor && !(SLOT_ARMOR in obscured))
		var/str = "[m3] [wear_armor.get_examine_overview(user)]. "
...
//		. += "[m3] [back.get_examine_string(user)] on [m2] back."
	...
	if(cloak && !(SLOT_CLOAK in obscured))
		...
		if(istype(cloak, /obj/item/clothing))
			...
			str = "[m3] [CL.get_examine_overview(user)] on [m2] shoulders. "
		else
	...
	if(gloves && !(SLOT_GLOVES in obscured))
		var/str = "[m3] [gloves.get_examine_overview(user)] on [m2] hands. "
	...
	if(shoes && !(SLOT_SHOES in obscured))
		var/str = "[m3] [shoes.get_examine_overview(user)] on [m2] feet. "
	...
	if(wear_mask && !(SLOT_WEAR_MASK in obscured))
		var/str = "[m3] [wear_mask.get_examine_overview(user)] on [m2] face. "
	...
	if(mouth && !(SLOT_MOUTH in obscured))
		...
		if(istype(mouth, /obj/item/clothing))
			...
			str = "[m3] [CM.get_examine_overview(user)] in [m2] mouth. "
		else
	...
	if(wear_neck && !(SLOT_NECK in obscured))
		var/str = "[m3] [wear_neck.get_examine_overview(user)] around [m2] neck. "
	...
	if(wear_ring && !(SLOT_RING in obscured))
		var/str = "[wear_ring.get_examine_overview(user)] on [m2] hands. "
	...
	if(wear_wrists && !(SLOT_WRISTS in obscured))
		var/str = "[m3] [wear_wrists.get_examine_overview(user)] on [m2] wrists."
